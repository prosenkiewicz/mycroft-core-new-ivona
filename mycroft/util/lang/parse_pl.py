...
from mycroft.util.lang.common_data_pl import _ARTICLES, _NUM_STRING_PL, \
    _LONG_ORDINAL_STRING_PL, _LONG_SCALE_PL, \
    _SHORT_SCALE_PL, _SHORT_ORDINAL_STRING_PL
...
_MULTIPLIES_LONG_SCALE_PL = set(_LONG_SCALE_PL.values()) | \
                            _generate_plurals(_LONG_SCALE_PL.values())

_MULTIPLIES_SHORT_SCALE_PL = set(_SHORT_SCALE_PL.values()) | \
                             _generate_plurals(_SHORT_SCALE_PL.values())
...
_STRING_NUM_PL = _invert_dict(_NUM_STRING_PL)
_STRING_NUM_PL.update(_generate_plurals(_STRING_NUM_PL))
...
_STRING_SHORT_ORDINAL_PL = _invert_dict(_SHORT_ORDINAL_STRING_PL)
_STRING_LONG_ORDINAL_PL = _invert_dict(_LONG_ORDINAL_STRING_PL)
...
def _extract_number_with_text_pl_helper(tokens,
    ...
    """
    Helper for _extract_number_with_text_pl
...
def _extract_decimal_with_text_pl(tokens, short_scale, ordinals):
    ...
    Notes:
        While this is a helper for extractnumber_pl, it also depends on
        extractnumber_pl, to parse out the components of the decimal.
...
def _extract_whole_number_with_text_pl(tokens, short_scale, ordinals):
    ...
    for idx, token in enumerate(tokens):
        ...
        if word not in string_num_scale and \
                ...
                not isFractional_pl(word, short_scale=short_scale) and \
        ...
        # half cup
        if val is False:
            val = isFractional_pl(word, short_scale=short_scale)
            current_val = val

        # 2 fifths
        if not ordinals:
            next_val = isFractional_pl(next_word, short_scale=short_scale)
            if next_val:
                if not val:
                    val = 1
                val = val * next_val
...
def _initialize_number_data(short_scale):
    ...
    """
    multiplies = _MULTIPLIES_SHORT_SCALE_PL if short_scale \
        else _MULTIPLIES_LONG_SCALE_PL

    string_num_ordinal_pl = _STRING_SHORT_ORDINAL_PL if short_scale \
        else _STRING_LONG_ORDINAL_PL

    string_num_scale_pl = _SHORT_SCALE_PL if short_scale else _LONG_SCALE_PL
    string_num_scale_pl = _invert_dict(string_num_scale_pl)
    string_num_scale_plupdate(_generate_plurals(string_num_scale_pl))

    return multiplies, string_num_ordinal_pl, string_num_scale_pl


def extractnumber_pl(text, short_scale=True, ordinals=False):
    """
    ...
    """
    return _extract_number_with_text_pl(_tokenize(text),
...
def extract_datetime_pl(string, dateNow, default_time):
    ...
    for idx, word in enumerate(words):
        ...
        if word == "now" and not datestr:
            ...
            multiplier = None
            if is_numeric(word):
                multiplier = extractnumber_pl(word)
...
def isFractional_pl(input_str, short_scale=True):
    ...
    if short_scale:
        for num in _SHORT_ORDINAL_STRING_PL:
            if num > 2:
                fracts[_SHORT_ORDINAL_STRING_PL[num]] = num
    else:
        for num in _LONG_ORDINAL_STRING_PL:
            if num > 2:
                fracts[_LONG_ORDINAL_STRING_PL[num]] = num
...
def normalize_pl(text, remove_articles):
